...
namespace Bourse.Controllers
{
    ...
    public class BourseController : Controller
    {
        ...
        [Authorize]
        public async Task<IActionResult> ForecastsDTOApi(string filtre, string exchangeFiltre, string sortOrder, int page = 1, int pageSize = 50, bool resethistorique = false)
        {
            if (resethistorique)
            {
                using (StreamWriter sw = new StreamWriter(_cheminFichier))
